<template>
  <div>
    <button class="btn btn-edit btn-outline-secondary" @click="exibirPopup">
      <span>📅</span>
    </button>
    <div class="overlay" v-if="exibir">
      <div id="popup" v-show="exibir" class="popup-container">
        <article>
          <h5 style="color: black;">PERIODO DO</h5>
          <h6 style="color: black;">CRONOGRAMA</h6>
        </article>
    ...
    async salvar() {
      try {
        ...
        const response = await fetch(
          `http://localhost:8080/api/obra/${idObra}`
        );
        if (!response.ok) {
        ...
        if (!updateResponse.ok) {
          throw new Error(
            "Erro na requisição. Código: " + updateResponse.status
          );
        }
